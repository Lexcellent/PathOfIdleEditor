...
- 角色成长：可直接编辑每级属性成长，单项上下限与总点数由当前职业、品级的游戏规则实时校验；也可通过监牢原生流程消耗血肉结晶重新随机。
- 额外技能：异化技能上限按角色品级和建筑属性动态读取（当前原生规则最高 2）；启迪天赋上限按神殿属性动态读取，二者均显示在技能表中。
- “同步异化技能”会按当前容量清理无效或超额项，并由游戏随机补齐；“启迪天赋”会调用神殿原生流程消耗材料并随机增加一个其他职业天赋。
- 崇拜者：独立页面编辑崇拜者等级；职业魔偶按实际修改项分别提交，可编辑等级、职业独立的力量/敏捷/智力范围，以及已有技能/专精等级加成。
